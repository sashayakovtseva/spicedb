...
func (y *ydbDatastore) watch(
	...
	cdcEvents, cdcErrors, err := y.cdcEvents(ctx, afterRevision, tableToTopicName, opts)
	if err != nil {
		errs <- fmt.Errorf("failed to read cdc events: %w", err)
		return
	}
	...
	sendChange := func(change *datastore.RevisionChanges) bool {
		...
		case updates <- change:
			return true
		case <-timer.C:
			sendError(datastore.NewWatchDisconnectedErr())
			return false
		}
	}
	...
	hackTicker := time.NewTicker(time.Minute * 5)
	defer hackTicker.Stop()
	...
	for {
		...
		case <-ctx.Done():
			...
			return
		case err := <-cdcErrors:
			sendError(fmt.Errorf("failed to read cdc events: %w", err))
			return
		case event, ok := <-cdcEvents:
			...
			switch topicToTableName[event.Topic] {
			case tableRelationTuple:
				if len(event.Key) != 7 {
					err := spiceerrors.MustBugf("unexpected PK size. want 7, got %d (%v)", len(event.Key), event.Key)
					sendError(err)
					return
				}
				...
				if err := tracked.AddRelationshipChange(
					ctx,
					createRev,
					tuple,
			...
			case tableNamespaceConfig:
				if len(event.Key) != 2 {
					err := spiceerrors.MustBugf("unexpected PK size. want 2, got %d (%v)", len(event.Key), event.Key)
					sendError(err)
					return
				}
			...
			if changesCount == expectedChangesCount {
				...
				// WARNING! This is for test purpose only and only for namespace-aware schema watch!
				// todo Fix this when 'resolved'-like messages are supported.
				futureCheckpoint := revisions.NewForTime(changesRevision.Time().Add(time.Minute * 30))
		...
		case now := <-hackTicker.C:
			if opts.Content&datastore.WatchCheckpoints == datastore.WatchCheckpoints {
				if !sendChange(&datastore.RevisionChanges{
					Revision:     revisions.NewForTime(now.Add(time.Minute * 30)),
					IsCheckpoint: true,
				}) {
					return
				}
...
func (y *ydbDatastore) cdcEvents(
	ctx context.Context,
	afterRevision datastore.Revision,
	tableToTopicName map[string]string,
	opts datastore.WatchOptions,
) (<-chan cdcEvent, <-chan error, error) {
	afterTimestampRevision, ok := afterRevision.(revisions.TimestampRevision)
	if !ok {
		return nil, nil, fmt.Errorf("expected timestamp revision, got  %T", afterRevision)
	}
	...
	if len(topics) == 0 {
		return nil, nil, fmt.Errorf("at least relationships or schema must be specified")
	}
	...
	for _, topic := range topics {
		selectors = append(selectors, topicoptions.ReadSelector{Path: topic, ReadFrom: readFromTime})
	...
	if err := reader.WaitInit(ctx); err != nil {
		return nil, nil, fmt.Errorf("failed to await reader initialization: %w", err)
	}

	messages := make(chan cdcEvent, 1)
	...
	go func() {
		...
		defer func() {
			...
			if err := reader.Close(closeCtx); err != nil {
				errCh <- fmt.Errorf("failed to close reader: %w", err)
			}
		}()
		...
		defer func() {
			...
			defer cancel()
			for _, topic := range topics {
				err := y.driver.Topic().Alter(
					ctx,
					topic,
					topicoptions.AlterWithDropConsumers(consumerUUID),
				)
				if err != nil {
					errCh <- fmt.Errorf("failed to remove consumer %q: %w", consumerUUID, err)
				}
			}
		}()
		...
		for msg, rErr = reader.ReadMessage(ctx); rErr == nil; msg, rErr = reader.ReadMessage(ctx) {
			...
			if err := topicsugar.JSONUnmarshal(msg, &event); err != nil {
				errCh <- fmt.Errorf("failed to unmarshal cdc event: %w", err)
				return
			}
			...
			if err := reader.Commit(ctx, msg); err != nil {
				errCh <- fmt.Errorf("failed to commit offset: %w", err)
				return
			}
		}

		if rErr != nil && !errors.Is(ctx.Err(), context.Canceled) {
			errCh <- fmt.Errorf("failed to read cdc events: %w", rErr)
		}
	}()
